...
export default function ProfilePage() {
  ...
  const achievements = [
    { icon: '🎯', name: 'Style Master', description: 'Created 50 outfits' },
    {
      icon: '⭐',
      name: 'Fashion Star',
      description: 'High ratings for 30 days',
    },
  ...
  const stats = [
    {
      ...
      value: state.outfits.length.toString(),
      icon: '👗',
    },
    {
      label: 'Items in Wardrobe',
      value: state.wardrobe.length.toString(),
      icon: '👚',
    },
    {
      label: 'Favorite Looks',
      value: state.wardrobe.filter((item) => item.favorite).length.toString(),
      icon: '❤️',
    },
    {
      label: 'Days Styled',
    ...
    <div className="px-4 pt-6 pb-24 space-y-6">
      ...
      <Card className="p-6 text-center bg-gradient-to-br from-purple-50 to-pink-50">
        ...
        <h1 className="text-2xl text-gray-800 mb-2">
          {state.user.name || 'Fashion Enthusiast'}
        </h1>
        ...
        <div className="flex items-center justify-center space-x-2 mb-4">
          {state.wardrobe.length > 0 ? (
            ...
            <Badge className="bg-gray-100 text-gray-700">Getting Started</Badge>
          )}
        </div>
        <Button className="bg-purple-500 hover:bg-purple-600 text-white">
          <Settings size={16} className="mr-2" />
          {state.user.name ? 'Edit Profile' : 'Set Up Profile'}
        </Button>
      </Card>
