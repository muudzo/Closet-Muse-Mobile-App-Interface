...
import { Plus, Search, Filter, Upload, Heart, Trash2, Edit, Camera, X } from 'lucide-react';
...
export default function WardrobeManager() {
  ...
  const categories = [
    ...
    { id: 'tops', label: 'Tops', icon: '👚', count: state.wardrobe.filter(item => item.category === 'tops').length },
    { id: 'dresses', label: 'Dresses', icon: '👗', count: state.wardrobe.filter(item => item.category === 'dresses').length },
    { id: 'bottoms', label: 'Bottoms', icon: '👖', count: state.wardrobe.filter(item => item.category === 'bottoms').length },
    { id: 'shoes', label: 'Shoes', icon: '👠', count: state.wardrobe.filter(item => item.category === 'shoes').length },
    { id: 'accessories', label: 'Accessories', icon: '👑', count: state.wardrobe.filter(item => item.category === 'accessories').length },
    { id: 'perfumes', label: 'Perfumes', icon: '🌸', count: state.wardrobe.filter(item => item.category === 'perfumes').length }
  ];
  ...
  const handleAddItem = () => {
    if (!newItem.name || !newItem.category || !newItem.color || !newItem.brand) {
  ...
  const toggleFavorite = (itemId: string) => {
    ...
    const item = state.wardrobe.find(i => i.id === itemId);
    toast.success(`${item?.name} ${item?.favorite ? 'removed from' : 'added to'} favorites`);
  };
    ...
    <div className="px-4 pt-6 pb-24 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewItem(prev => ({ ...prev, category: value as WardrobeItem['category'] }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select 
                    value={newItem.style} 
                    onValueChange={(value) => setNewItem(prev => ({ ...prev, style: value as WardrobeItem['style'] }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => setNewItem(prev => ({ ...prev, favorite: e.target.checked }))}
                  className="rounded"
                />
      ...
      <Card className="p-6 border-2 border-dashed border-purple-200 bg-purple-50/30">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">Upload photos of your clothes to get AI-powered styling suggestions</p>
          <input
            ref={fileInputRef}
            type="file"
            multiple
        ...
        {filteredItems.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            {filteredItems.map((item) => (
              <Card key={item.id} className="p-4 hover:shadow-lg smooth-transition">
                <div className="relative">
                  ...
                  <button
                    ...
                    className={`absolute top-2 right-2 p-1 rounded-full smooth-transition ${
                      item.favorite ? 'bg-red-100 text-red-500' : 'bg-white text-gray-400 hover:text-red-400'
                    }`}
                  >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-purple-600">{item.color}</span>
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            <p className="text-2xl text-teal-600 mb-1">
              {Math.round(state.wardrobe.reduce((acc, item) => acc + item.timesWorn, 0) / state.wardrobe.length) || 0}
            </p>
